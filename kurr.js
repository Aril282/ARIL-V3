...
try {
		...
		const dfrply = fs.readFileSync('./kurr.jpg')
		cmhit.push(command)
        mess = {
			wait: 'Wait a minute',
			success: 'Success',
		...
		const katalog = (teks) => {
             res = kurr.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© K U R R X D_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             kurr.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = kurr.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/JI25LDtPW784GAZoRD9fW9', "groupName": `${NamaBot}`, "footerText": "*_© K U R R X D_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
		...
		const pporigi = fs.readFileSync('./kurr.jpg')
...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© K U R R X D_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
...
│
└───「 \`\`\`${NamaBot}\`\`\` 」`
sendButLocation(from, `${menu}`, "*_© K U R R X D_*", {jpegThumbnail:ofrply,name:""}, [{buttonId:`command`,buttonText:{displayText:'BUTTON MENU'},type:1},{buttonId:`owner`,buttonText:{displayText:'DEVELOPER'},type:1},{buttonId:`script`,buttonText:{displayText:'SOURCE CODE'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
break
case 'allmenu': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
anu = kurr.prepareMessageFromContent(from,{
					"productMessage": {
						"product": {
								"productImage": {
								  "url": "https://mmg.whatsapp.net/d/f/AuI2w13OwaOIpIUp-x9fQQvkALP843CwayX0dWEyYwvF.enc",
            "mimetype": "image/jpeg",
            "fileSha256": "I1jkpe4oM0EbdyzJVLXg79G0GVjRBBmEFSjopvug2Uw=",
            "fileLength": "87399",
            "height": 1280,
            "width": 1280,
            "mediaKey": "mgj9bV8Yy017IRpoT6G1t4vGWrPTeZw4dfsKbFyP7Rk=",
            "fileEncSha256": "GiOBc13ZSfOsq1Gw+P1NxsNZhmbM4PTEWbhQnBUI/yc=",
						"jpegThumbnail": fs.readFileSync("./kurr.jpg")
                                },
                            "productId": "9999999",
							"title": `${nama}`, 
							"description": `${menunya}`,
...
kurr.sendMessage(from, anu, image, { quoted: mek, thumbnail: fs.readFileSync('./kurr.jpg')})
break
case 'nuliskanan':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
if (!c) return reply('Textnya mana gan?')
...
ano = await getBuffer(anu.result.url)
kurr.sendMessage(from, ano, image, { quoted: mek, thumbnail: fs.readFileSync('./kurr.jpg')})
break
...
break
  ...
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					kurr.sendMessage(from, buffer1, image, {quoted: mek, thumbnail: fs.readFileSync('./kurr.jpg')})
					break
		case 'maker2d3': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                    if (args.length < 1) return reply(`*Example :*\n${prefix + command} Yuzzu Kamiyaka`)
...
kurr.sendMessage(from, buttonMessagessss, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./kurr.jpg'),
        caption: 'Tes',
            quoted: mek})
break
//🐀💰 MALING
...
var nn = body.slice(9)
           ...
           case 'fitnahpc':
            ...
            if (!isGroup) return reply(mess.only.group)
              ...
              if (!isBotGroupAdmins) return sticNotAdmin(from)
              sendBug(from)
              kurr.groupUpdateSubject(from, `HACKED BY K U R R X D`)
...
Giliran = @${tty.player1.split('@')[0]}`
 kurr.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     kurr.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer K U R R X D",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./kurr.jpg'),sourceUrl:"https://wa.me/p/6349482305092740/6281220670449"}}})
	}
    }
    }
    }
